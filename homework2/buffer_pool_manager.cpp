 ...
 */
    Page *BufferPoolManager::FetchPage(page_id_t page_id) {
        ...
        Page *tar = nullptr;
        if (page_table_->Find(page_id,tar)) { //1.1找到页面
            tar->pin_count_++;//页面引用次数加一
            replacer_->Erase(tar);
            return tar;
        }
        //1.2没有找到,得到一个VictimPage and tar is null,return tar
        tar = GetVictimPage();
        if (tar == nullptr)
            return tar;
        //2页面被修改，需要写会
